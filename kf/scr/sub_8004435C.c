...
void sub_8004435C(uint8_t* rdram, recomp_context* ctx) {
    ...
    ctx->r7 = 0 | 0X5DC0;
    // lh          $a1, 0x26($s1)
    ctx->r5 = MEM_HS(0X26, ctx->r17);
    // lh          $a2, 0x24($s1)
    ctx->r6 = MEM_HS(0X24, ctx->r17);
    // addiu       $v0, $sp, 0x40
...
L_80044408:
    ...
    ctx->r16 = S32(ctx->r16) >> 16;
    // lh          $a0, 0x24($s1)
    ctx->r4 = MEM_HS(0X24, ctx->r17);
    // lh          $a1, 0x30($sp)
    ctx->r5 = MEM_HS(0X30, ctx->r29);
    // jal         0x80014C84
    // addu        $a2, $s0, $zero
    ...
    after_2:
    // lh          $a0, 0x26($s1)
    ctx->r4 = MEM_HS(0X26, ctx->r17);
    // sh          $v0, 0x24($s1)
    MEM_H(0X24, ctx->r17) = ctx->r2;
    // lh          $a1, 0x32($sp)
    ctx->r5 = MEM_HS(0X32, ctx->r29);
    // jal         0x80014C84
    // addu        $a2, $s0, $zero
    ctx->r6 = ADD32(ctx->r16, 0);
    sub_80014C84(rdram, ctx);
