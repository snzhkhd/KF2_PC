...
void KF_SpuKeyOnVoice(uint8_t* rdram, recomp_context* ctx) 
{
    ...
    uint16_t pitch = MEM_HU(0, 0x8019E6E8);
    uint8_t vol_main = MEM_BU(0, 0x8019E6DA);
    uint8_t vol_vel = MEM_BU(0, 0x8019E6DD);
    uint8_t pan = MEM_BU(0, 0x8019E6DE);
    uint8_t adsr_ar = MEM_BU(0, 0x8019E6DF);
    uint8_t adsr_dr = MEM_BU(0, 0x8019E6E0);
    uint8_t adsr_sl = MEM_BU(0, 0x8019E6E1);
    uint8_t adsr_rr = MEM_BU(0, 0x8019E6E2);
    uint8_t adsr_sr = MEM_BU(0, 0x8019E6E3);

    uint32_t spu_addr = MEM_W(0, 0x801DEFA4 + voice * 4);

    int vol_l = vol_main * vol_vel * 129 / 0x7F;
    int vol_r = vol_l;
    if (pan >= 0x40) {
        vol_l = vol_l * (127 - pan) / 0x3F;
    }
    else {
        vol_r = vol_r * pan / 0x3F;
    }

    printf("[SPU KeyOn] voice=%d pitch=%04X vol=%d/%d spu=%08X\n",
        voice, pitch, vol_l, vol_r, spu_addr);
    ...
    attr.mask = SPU_VOICE_VOLL | SPU_VOICE_VOLR | SPU_VOICE_PITCH |
        ...
        SPU_VOICE_ADSR_SL | SPU_VOICE_ADSR_SMODE |
        SPU_VOICE_ADSR_SR | SPU_VOICE_ADSR_RMODE |
        SPU_VOICE_ADSR_RR;
