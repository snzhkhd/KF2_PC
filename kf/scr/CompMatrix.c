...
void KF_CompMatrix(uint8_t* rdram, recomp_context* ctx) {
    ...
    // lhu         $t5, 0x14($a1)
    ctx->r13 = MEM_HU(0X14, ctx->r5);
    // lw          $t6, 0x18($a1)
    ctx->r14 = MEM_W(0X18, ctx->r5);
    // lw          $t2, 0x1C($a1)
    ctx->r10 = MEM_W(0X1C, ctx->r5);
    // sll         $t6, $t6, 16
    ctx->r14 = S32(ctx->r14) << 16;
    // or          $t5, $t5, $t6
    ...
    // lw          $t3, 0x14($a0)
    ctx->r11 = MEM_W(0X14, ctx->r4);
    // lw          $t4, 0x18($a0)
    ctx->r12 = MEM_W(0X18, ctx->r4);
    // lw          $t5, 0x1C($a0)
    ctx->r13 = MEM_W(0X1C, ctx->r4);
    // add         $t0, $t0, $t3
