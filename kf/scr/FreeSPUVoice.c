...
{   
    ...
    uint16_t voice = (uint16_t)ctx->r4;
    uint8_t active = MEM_BU(0, 0x8019E6F0 + voice);

    //static int free_log = 0;
    //if (free_log++ < 30) {
    //    printf("[FreeSPUVoice] voice=%d active=%d\n", voice, active);
    //}
    ...
    if (active == 1) {
        MEM_B(0, 0x8019E6F0 + voice) = 0;
        uint16_t count = MEM_HU(0, 0x801DEF9C);
        if (count > 0) count--;
        MEM_H(0, 0x801DEF9C) = count;
        ctx->r2 = count;
    }
    else {
        ctx->r2 = 0;
    ...
    if (!bDone)
    {
        printf("[FreeSPUVoice] voice=%d caller=%08X\n", voice, ctx->r31);
        bDone = true;
    }
