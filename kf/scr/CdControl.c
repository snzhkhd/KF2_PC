    ...
    if (cmd == 0x16) // CdlReadN — запуск стримингового чтения
    {
        ...
        if (p_active && *p_active) {
            ...
            if (stream)
            {
                ...
                //// Вызываем обработчик по маске stream[0]
                //uint8_t mask = stream[0]; // 0x10, 0x20, 0x40 и т.д.
                ...
                //switch (task_type) {
                //case 0x10: handler = lookup_recomp_func(0x80017DB4); break; // тайлмап
                //case 0x20: handler = lookup_recomp_func(0x80017F2C); break; // SPU аудио
                ...
                //    // Симулируем DMA interrupt loop
                //    // Первый вызов: stream[1]=0 → читает данные → stream[1]=1, stream[36]=1
                //    // Второй вызов: stream[1]=1 → VerifyChecksum → вызов колбэка
                //    for (int iter = 0; iter < 4; iter++) {
                //        if (!stream[36]) break;
                //        stream[36] = 0; // сбрасываем перед вызовом
                //        ctx->r4 = *p_active;
                //        handler(rdram, ctx);
